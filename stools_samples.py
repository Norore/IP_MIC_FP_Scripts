...
for f_stool in l_files:
    ...
    # split column Box
    newcols = pd.DataFrame(df_stool.Box.str.split(",").tolist(), columns=["Position", "Box", "Name"])
    ...
    # need to know the corresponding Level3
    ...
    boxes = pd.DataFrame(d_box.items(), columns=["Box", "Level3"], dtype=object)
    ...
    # select lines in freezers with "stools" in column "Level2"
    stools = df_freez.loc[df_freez["Level2"].str.contains("Stool samples Aliquot")]

    # merge stools data with freezer location
    dataset.rename(columns={"Shelf": "Level1"}, inplace=True)
    result = pd.merge(stools, dataset, on=["Freezer", "Level1", "Level3", "Box"], how="inner")
